...
2. Поля (англ. / рус.):
   ...
   - **DNS**: `10.10.0.1` (это поле для **пиров**, не для самого хаба. Пиры получат этот DNS в свой `wg0.conf`).
     ⚠️ **Только IP-адрес, без маски CIDR.** Не `10.10.0.1/32` — иначе wg-portal не сохранит и в конфиге пиров строка `DNS = ` будет пустой. Несколько резолверов через запятую: `10.10.0.1, 1.1.1.1`.
...
```bash
# Сначала установи root CA от секутора (один раз) — чтобы acme.sh доверял
# bootstrap-cert'у ACME-эндпоинта:
curl -kL https://acme.lan.vpn:8443/ca.pem | \
  sudo tee /usr/local/share/ca-certificates/secutor-root.crt > /dev/null
sudo update-ca-certificates

# Выпуск
acme.sh --issue -d wg.lan.vpn --standalone \
  --server https://acme.lan.vpn:8443/directory
...
### В скачанном конфиге пустое поле `DNS = `

Скорее всего, в поле DNS интерфейса/пира ты ввёл адрес с маской CIDR (`10.10.0.1/32`). Поле принимает **только IP без маски**, такой ввод молча отклоняется или сохраняется невалидно.

Правильно:
- ✅ `10.10.0.1`
- ✅ `10.10.0.1, 1.1.1.1` (несколько через запятую)
- ❌ `10.10.0.1/32`
- ❌ `10.10.0.1:53` (порт указывать нельзя)

После исправления у новых пиров строка `DNS =` появится сразу. Для существующих — пересохрани каждого пира (или примени defaults), затем перевыпусти конфиги.

Если даже без маски не сохраняется — правь напрямую в БД:
```bash
docker compose exec wg-portal sqlite3 /app/data/sqlite.db
```
```sql
UPDATE peers SET dns_str = '10.10.0.1' WHERE identifier = '<твой-пир>';
.quit
```
Перед SQL — бэкап: `cp data/sqlite.db data/sqlite.db.bak`.
